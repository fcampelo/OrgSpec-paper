...
# Organisms tested
orgs     <- dir("./", pattern = "[1-9]+_")
orgnames <- sapply(orgs, function(x)strsplit(x, split = "_")[[1]][2])
...
bepipred.data <- bepipred_ids %>%
  dplyr::left_join(epitopes, by = c("ID" = "epitope_id")) %>%
...
org.list <- data.frame(Name = orgnames,
                       path = orgs)
...
for (i in 1:nrow(org.list)){
  ...
  tr.seqs   <- epitopes$epit_seq[epitopes$epitope_id %in% tr.ids]
  ...
  ho.seqs  <- epitopes[epitopes$epitope_id %in% ho.ids, ]
  ho.proteins <- proteins$TSeq_sequence[proteins$UID %in% unique(ho.data$Info_protein_id)]
