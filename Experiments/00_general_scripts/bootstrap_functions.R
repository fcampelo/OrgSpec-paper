...
# Base function to calculate bootstrap confidence intervals on performance:
# if is.na(i) then performance is calculated using the original data, 
# otherwise a bootstrat resample is done prior to performance calculations.
boot_performance <- function(i, df, which.cols, which.prob.cols = NULL){
  idx <- 1:nrow(df)
...
# Create a single estimate of paired performance differences:
